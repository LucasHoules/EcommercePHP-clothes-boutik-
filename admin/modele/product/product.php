<?php
function get_produits($parent){
  global $db;
  $parent = (int) $parent;
  $req = $db->prepare('SELECT * FROM produits WHERE deleted = :parent');
  $req->bindParam(':parent', $parent, PDO::PARAM_INT);
  $req->execute();
  $produits = $req->fetchAll();
  return $produits;
}
function updateFeatured($featured, $id ){
  global $db;
  $req = $db->prepare('UPDATE produits SET featured = :featured WHERE id = :id');
  $req->bindParam(':featured' , $featured, PDO::PARAM_INT);
